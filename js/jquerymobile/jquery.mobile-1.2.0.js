...
$.fn.buttonMarkup = function( options ) {
	...
	for ( var i = 0; i < $workingSet.length; i++ ) {
		...
		if($("#curentPageId").val() == 'editPage_doctor')
		{
			buttonText.id = textClass+'dr'+attempDR; attempDR++;
		}
		
		if($("#curentPageId").val() == 'editPage_chemist')
		{
			buttonText.id = textClass+'ch'+attempCH; attempCH++;
		}		
...
$.widget( "mobile.button", $.mobile.widget, {
	options: {
		...
		shadow: true,
		iconshadow: true,
		initSelector: "button, [type='button'], [type='reset']"
	},
	_create: function() {
		var $el = this.element,
			$button,
